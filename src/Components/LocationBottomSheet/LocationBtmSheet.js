...
const Location = ({ location, bottomSheetRef, setSelected }) => {
  ...
  const toggleLocationSelection = (locationValue) => {
    if (selectedFriends === locationValue) {
      setSelectedFriends();
      setSelected(locationValue);
    } else {
      setSelectedFriends(locationValue);
    }
  };
      ...
      <BottomSheetFlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() => toggleLocationSelection(item.BranchID)}
          >
            <View style={styles.friendInfo}>
              <Text style={styles.locationName}>{item.Branch}</Text>
            </View>
            {selectedFriends === item.BranchID && <CheckBoxBlueTick width={wp(6)}  /> }
          </TouchableOpacity>
        )}
        scrollEnabled={true}
      />
