...
const Location = ({ location, bottomSheetRef, setSelected, selectedLocation, setFieldValue }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFriends, setSelectedFriends] = useState(selectedLocation);
  ...
  const toggleLocationSelection = (location) => {
    setSelectedFriends(location); // Always select the location
    setSelected(location);        // Update the selected state
    setFieldValue('location', location.BranchId);
    // getInvites({
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
       ...
       <TxtInput placeholder={'Search'} selectableColor={COLORS.black} placeholderTextColor={COLORS.darkGraytext} containerStyle={[styles.txtInput]} svg={<Search/>}  onChangeText={setSearchQuery} />
      </View>
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.friendInfo}>
              <Text style={styles.locationName}>{item.CompanyWithBranch}</Text>
            </View>
            {selectedFriends && selectedFriends.BranchId === item.BranchId && <CheckBoxBlueTick width={wp(6)}  />}
          </TouchableOpacity>
        )}
        scrollEnabled={true}
      />
