...
const WindowArrivalBtmSheet = ({ data, bottomSheetRef, setSelected, setFormValues , formValues }) => {
  ...
  const toggleLocationSelection = (item) => {
    ...
    // setFieldValue('arrivalWindow', item.Key);
    setFormValues({...formValues, arrivalWindow: item.Key})
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.friendInfo}>
              <Text style={styles.locationName}>{item.Key} mins</Text>
            </View>
            {selectedFriends && selectedFriends.Key === item.Key && <CheckBoxBlueTick width={wp(6)}  />}
          </TouchableOpacity>
        )}
        scrollEnabled={true}
      />
...
export default WindowArrivalBtmSheet;
